...
DATASET_TRAINING_IMAGE_FOLDER = 'imagesTr'
DATASET_TRAINING_MASK_FOLDER = 'labelsTr'
DATASET_TEST_IMAGE_FOLDER = 'imagesTs'
DATASET_TEST_MASK_FOLDER = 'labelsTs'
...
def prepare_dataset(root_dir, target_dir, create_masks=True):
    ...
    # Build helper maps for moving corresponding files
    def _stem(path: str) -> str:
        return os.path.splitext(os.path.basename(path))[0]

    # Map from case stem -> dicom path (if present)
    dicom_by_stem = {}
    for dp in all_images_dicom:
        dicom_by_stem[_stem(dp)] = dp

    # Ensure test folders exist
    os.makedirs(os.path.join(target_dir, DATASET_TEST_IMAGE_FOLDER), exist_ok=True)
    os.makedirs(os.path.join(target_dir, DATASET_TEST_MASK_FOLDER), exist_ok=True)

    # --------------------------------------------------
    # Train/Test split (10% test, seed=42, stratified)
    # --------------------------------------------------
    all_images = []
    all_labels = []

    for p in nonstroke_images_png:
        all_images.append(p)
        all_labels.append(0)

    for p in ischemic_images_png:
        all_images.append(p)
        all_labels.append(1)

    for p in hemorrhagic_images_png:
        all_images.append(p)
        all_labels.append(2)
    ...
    # --------------------------------------------------
    # Physically move TEST items to imagesTs/labelsTs
    # (so the folder structure matches the split)
    # --------------------------------------------------
    def _move_if_exists(src_path: str, dst_path: str):
        if os.path.exists(src_path):
            os.makedirs(os.path.dirname(dst_path), exist_ok=True)
            # If destination exists, overwrite by removing first
            if os.path.exists(dst_path):
                os.remove(dst_path)
            shutil.move(src_path, dst_path)

    # Move test PNG + corresponding label + corresponding DICOM (if present)
    for p in test_images.tolist():
        stem = _stem(p)  # e.g., "10000"

        # Move PNG image
        src_png = p
        dst_png = os.path.join(target_dir, DATASET_TEST_IMAGE_FOLDER, os.path.basename(p))
        _move_if_exists(src_png, dst_png)

        # Move label
        src_lbl = p.replace(DATASET_TRAINING_IMAGE_FOLDER, DATASET_TRAINING_MASK_FOLDER)
        dst_lbl = os.path.join(target_dir, DATASET_TEST_MASK_FOLDER, os.path.basename(src_lbl))
        _move_if_exists(src_lbl, dst_lbl)

        # Move dicom with same stem if it exists
        if stem in dicom_by_stem:
            src_dcm = dicom_by_stem[stem]
            dst_dcm = os.path.join(target_dir, DATASET_TEST_IMAGE_FOLDER, os.path.basename(src_dcm))
            _move_if_exists(src_dcm, dst_dcm)

    # After moving, rebuild the train/test file paths to reflect the new locations
    train_images = np.array([
        os.path.join(target_dir, DATASET_TRAINING_IMAGE_FOLDER, os.path.basename(p))
        for p in train_images
    ])
    test_images = np.array([
        os.path.join(target_dir, DATASET_TEST_IMAGE_FOLDER, os.path.basename(p))
        for p in test_images
    ])

    # Class-wise preservation
    train_nonstroke = train_images[train_labels == 0].tolist()
    train_ischemic = train_images[train_labels == 1].tolist()
    train_hemorrhagic = train_images[train_labels == 2].tolist()

    test_nonstroke = test_images[test_labels == 0].tolist()
    test_ischemic = test_images[test_labels == 1].tolist()
    test_hemorrhagic = test_images[test_labels == 2].tolist()
    test_anomaly = test_ischemic + test_hemorrhagic
    ...
    data = {
        ...
        "training": [
            {
                "image_png": os.path.relpath(p, target_dir),
                "image_dicom": os.path.relpath(
                    os.path.join(
                        target_dir,
                        DATASET_TRAINING_IMAGE_FOLDER,
                        os.path.splitext(os.path.basename(p))[0] + ".dcm",
                    ),
                    target_dir,
                )
                if os.path.exists(
        ...
        "training_ischemic": [
            ...
            for p in train_ischemic
        ],

        "training_hemorrhagic": [
            {
                "image_png": os.path.relpath(p, target_dir),
                "image_dicom": os.path.relpath(
                    os.path.join(
        ...
        "training_anomaly": [
            ...
            for p in (train_ischemic + train_hemorrhagic)
        ],
