...
def main():
    ...
    if opt.model == "ldm":
        ...
        cli = OmegaConf.from_dotlist(unknown)
        config = OmegaConf.merge(config, cli)    
    elif opt.model == "attention_unet":
        print("Evaluate on stroke dataset in segmentation manner using attention unet.")
        opt.config = "configs/unet/stroke-attention_unet.yaml"
        opt.outdir = "outputs/samples-stroke-attention_unet"
        opt.save_results = True
        dataset = StrokeValidation
        ...
        cli = OmegaConf.from_dotlist(unknown)
        config = OmegaConf.merge(config, cli)    
    elif opt.model == "transunet":
        print("Evaluate on stroke dataset in segmentation manner using transunet.")
        opt.config = "configs/unet/stroke-transunet.yaml"
        opt.outdir = "outputs/samples-stroke-transunet"
        opt.save_results = True
        dataset = StrokeValidation
    ...
    test_params = config["data"]["params"]["validation"]["params"]
    ...
    for idx in range(opt.times):
        ...
        dice_list = metrics_dict["val_avg_dice"][1:]
        iou_list = metrics_dict["val_avg_iou"][1:]
        tiou_list = metrics_dict["val_avg_tiou"][1:]
        print(f"\033[31m[Mean Dice (HS+IS)][{opt.model}]: {sum(dice_list) / len(dice_list)}\033[0m")
        print(f"\033[31m[Mean  IoU (HS+IS)][{opt.model}]: {sum(iou_list) / len(iou_list)}\033[0m")
        print(f"\033[31m[Mean  tIoU (HS+IS)][{opt.model}]: {sum(tiou_list) / len(tiou_list)}\033[0m")
