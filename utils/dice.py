...
class DiceLoss(_Loss):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        f: torch.Tensor = torch.where((ground_o + pred_o) > 0, 1.0 - (2.0 * intersection ) / (denominator), torch.tensor(float("nan"), device=ground_o.device))
        ...
        if self.reduction == LossReduction.MEAN.value:
            ...
            f = torch.nanmean(f) # the batch and channel average
