...
class TolerantMeanIoU(CumulativeIterationMetric):
    """
...
def compute_tolerant_iou(
    ...
    """
    Morphological tolerant IoU computation with dilation/erosion on ground truth.
    """
    ...
    def morph_op(inp, op_type):
        out = []
        for c in range(C):
            x = inp[:, c:c+1]
            conv = F.conv2d(x.float(), kernel, padding=pad)
            if op_type == "dilate":
                out.append((conv > 0).float())
            elif op_type == "erode":
                out.append((conv == kernel.numel()).float())
        return torch.cat(out, dim=1)

    dilated_y = morph_op(y, "dilate")
    eroded_y = morph_op(y, "erode")
    pred_bin = (y_pred > 0.5).float()

    intersection = torch.sum((pred_bin == dilated_y) * (dilated_y > 0), dim=(2, 3))
    union = torch.sum((eroded_y > 0) | (pred_bin > 0), dim=(2, 3))

    if ignore_empty:
        gt_sum = torch.sum(y, dim=(2, 3))
        iou = torch.where(gt_sum > 0, intersection / union, torch.tensor(float("nan"), device=device))
    else:
        iou = torch.where(union > 0, intersection / union, torch.tensor(1.0, device=device))

    return iou  # Shape: (B, C)