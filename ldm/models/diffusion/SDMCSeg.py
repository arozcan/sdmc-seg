...
from utils.image_processing_utils import overlay_images, colorize_multichannel_segment
...
class DDPM(pl.LightningModule):
    ...
    def p_losses(self, x_start, t, noise=None):
        ...
        if self.original_elbo_weight > 0:
            ...
            # Skip VLB computation when it is not used
            loss_dict.update({f'{log_prefix}/loss_vlb': 0.0})
            loss = loss_simple
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.use_scheduler:
            ...
            self.log('lr_abs', float(lr), prog_bar=True, logger=True, on_step=True, on_epoch=False, batch_size = batch_size)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Single validation forward for speed.
        # If EMA is enabled, validate using EMA weights only.
        if self.use_ema:
            with self.ema_scope():
    ...
    def log_images(self, batch, N=8, n_row=2, sample=True, return_keys=None, **kwargs):
        log = dict()
        # Flags passed from ImageLogger.log_images_kwargs
        plot_progressive_rows = bool(kwargs.pop("plot_progressive_rows", True))  # kept for API compatibility
        plot_diffusion_rows = bool(kwargs.pop("plot_diffusion_rows", True))
        plot_denoise_rows = bool(kwargs.pop("plot_denoise_rows", True))
        _ = kwargs.pop("inpaint", None)
        _ = kwargs.pop("ddim_steps", None)
        ...
        # get diffusion row (optional; can be very expensive)
        need_diffusion_row = plot_diffusion_rows or (return_keys is not None and "diffusion_row" in return_keys)
        if need_diffusion_row:
            diffusion_row = []
            x_start = x[:n_row]

            # iterate only over logged timesteps instead of all timesteps
            timesteps_to_log = list(range(0, self.num_timesteps, self.log_every_t))
            if (self.num_timesteps - 1) not in timesteps_to_log:
                timesteps_to_log.append(self.num_timesteps - 1)

            for tt in timesteps_to_log:
                tt = repeat(torch.tensor([tt]), '1 -> b', b=n_row)
                tt = tt.to(self.device).long()
                noise = torch.randn_like(x_start)
                x_noisy = self.q_sample(x_start=x_start, t=tt, noise=noise)
                diffusion_row.append(x_noisy)

            log["diffusion_row"] = self._get_rows_from_list(diffusion_row)

        if sample:
            # get samples/denoise row (optional)
            need_samples = (return_keys is None) or ("samples" in return_keys) or ("denoise_row" in return_keys)
            need_denoise_row = plot_denoise_rows or (return_keys is not None and "denoise_row" in return_keys)

            return_intermediates = bool(need_denoise_row)
            with self.ema_scope("Plotting"):
                out = self.sample(batch_size=N, return_intermediates=return_intermediates)

            if return_intermediates:
                samples, denoise_row = out
            else:
                samples, denoise_row = out, None

            if need_samples:
                log["samples"] = samples
            if denoise_row is not None:
...
class LatentDiffusion(DDPM):
    ...
    def instantiate_cond_stage(self, config):
        if not self.cond_stage_trainable:
            if config == "__is_first_stage__":
                ...
                self.cond_stage_model.encoder.eval()
                for param in self.cond_stage_model.encoder.parameters():
                    param.requires_grad = False

                # `out` kısmını eğitilebilir bırak
                for param in self.cond_stage_model.out.parameters():
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        if self.learn_logvar:
            ...
            # No logvar weighting when learn_logvar is disabled
            loss = loss_simple
        ...
        if self.original_elbo_weight > 0:
            ...
            # Skip VLB computation when it is not used
            loss_dict.update({f'{prefix}/loss_vlb': 0.0})
...
class SDSeg(LatentDiffusion):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log_dict(loss_dict, prog_bar=True,
                      logger=True, on_step=True, on_epoch=False, batch_size = batch_size)
        self.log("step", float(self.global_step),
                 prog_bar=True, logger=True, on_step=True, on_epoch=False, batch_size = batch_size)

        if self.use_scheduler:
            lr = self.optimizers().param_groups[0]['lr']
            self.log('param/lr_abs', float(lr.item()), prog_bar=False, logger=True, on_step=True, on_epoch=False, batch_size = batch_size)
    ...
    def on_train_batch_start(self, batch, batch_idx, dataloader_idx=0):
        ...
        if self.current_epoch == 0 and self.global_step == 0 and batch_idx == 0:  # and not self.restarted_from_ckpt:
            ...
            self.log("val_avg_dice", float(0), prog_bar=False, logger=True, on_step=True, on_epoch=False, batch_size = batch_size)
    ...
    def p_losses(self, x_start, cond, t, seg_label, noise=None):
        ...
        # add latent seg loss
        loss += loss_seg.mean()  # weight == 1 is good enough
        loss_dict.update({f'{prefix}/loss': loss})
...
class SDMCSeg(SDSeg):
    def __init__(self, first_stage_config, cond_stage_config, load_only_unet=True, num_classes=2, *args, **kwargs):
        super().__init__(first_stage_config, cond_stage_config, load_only_unet=load_only_unet,num_classes=num_classes, *args, **kwargs)
        self.val_dice = DiceMetric(include_background=True, reduction="mean", get_not_nans=False)
        self.val_iou = MeanIoU(include_background=True, reduction="mean", get_not_nans=False)
        self.val_tiou = TolerantMeanIoU(include_background=True, reduction="mean", get_not_nans=False)
        self.val_hd95 = HausdorffDistanceMetric(
            include_background=False, reduction="mean", percentile=95, get_not_nans=False
        )
    ...
    def log_dice(self, data=None, save_dir=None, ddim_steps=50):
        ...
        def get_dice(data, used_sampler="ddim", save_dir=None, ddim_steps=50):
            ...
            def get_dice_loop(data, sampler, use_direct=False, noise=None, save_dir=None, ddim_steps=50):
                ...
                # --- per-class lists (ignore_empty=True behavior via NaN) ---
                sens_list_by_cls = {1: [], 2: []}
                spec_list_by_cls = {1: [], 2: []}
                hd95_list_by_cls = {1: [], 2: []}
                ...
                for prompts in pbar:
                    slice_path = prompts["file_path_"]
                    image = prompts["image"]                 # 1 H W C
                    label = prompts["segmentation"]          # 1 H W C
                    assert image.shape == label.shape

                    label = torch.where(label > 0.5, 1.0, 0.0)
                    image_np, label_np = image.squeeze(0).numpy(), label.squeeze(0).numpy()
                    label = label.permute(0, 3, 1, 2).to(self.device)  # 1 C H W

                    c = dict(
                        c_concat=[self.get_learned_conditioning(prepare_for_first_stage(image))],
                        c_crossattn=[None]
                    )
                    ...
                    x_samples = self.decode_first_stage(samples_pred[0])
                    out_p = torch.clamp((x_samples + 1.0) / 2.0, min=0.0, max=1.0)  # 1 C H W
                    out = torch.where(out_p > 0.5, 1.0, 0.0)                         # 1 C H W

                    # --- binarized masks for metric math ---
                    out_b = (out > 0.5)
                    lab_b = (label > 0.5)

                    # ----------- Per-class Sens/Spec/HD95 (cls 1 & 2) -----------
                    for cls in (1, 2):
                        pred_c = out_b[:, cls:cls+1, ...]
                        gt_c   = lab_b[:, cls:cls+1, ...]

                        # ignore_empty=True behavior: if GT empty => NaN (ignored by nanmean)
                        if gt_c.sum().item() == 0:
                            sens_list_by_cls[cls].append(float("nan"))
                            spec_list_by_cls[cls].append(float("nan"))
                            hd95_list_by_cls[cls].append(float("nan"))
                            continue
                        ...
                        sens_list_by_cls[cls].append(sens)
                        spec_list_by_cls[cls].append(spec)

                        # HD95 for this class: 2-class one-hot (bg vs this class)
                        pred_fg = pred_c.float()
                        gt_fg   = gt_c.float()
                        pred_oh = torch.cat([1.0 - pred_fg, pred_fg], dim=1)
                        gt_oh   = torch.cat([1.0 - gt_fg,   gt_fg],   dim=1)
                        hd = hd95_metric(pred_oh, gt_oh)  # shape (1,) with reduction="none"
                        hd95_list_by_cls[cls].append(float(hd.squeeze().detach().cpu().numpy()))

                    # ----------- Union foreground (cls1 OR cls2) -----------
                    pred_fg_u = (out_b[:, 1:2, ...] | out_b[:, 2:3, ...])
                    gt_fg_u   = (lab_b[:, 1:2, ...] | lab_b[:, 2:3, ...])

                    if gt_fg_u.sum().item() == 0:
                        sens_list_fg.append(float("nan"))
                        spec_list_fg.append(float("nan"))
                        hd95_list_fg.append(float("nan"))
                    else:
                        tp = (pred_fg_u & gt_fg_u).sum().item()
                        fn = ((~pred_fg_u) & gt_fg_u).sum().item()
                        fp = (pred_fg_u & (~gt_fg_u)).sum().item()
                        tn = ((~pred_fg_u) & (~gt_fg_u)).sum().item()

                        sens_u = tp / (tp + fn + 1e-8)
                        spec_u = tn / (tn + fp + 1e-8)

                        sens_list_fg.append(sens_u)
                        spec_list_fg.append(spec_u)

                        pred_fg = pred_fg_u.float()
                        gt_fg   = gt_fg_u.float()
                        pred_oh = torch.cat([1.0 - pred_fg, pred_fg], dim=1)
                        gt_oh   = torch.cat([1.0 - gt_fg,   gt_fg],   dim=1)
                        hd_u = hd95_metric(pred_oh, gt_oh)
                        hd95_list_fg.append(float(hd_u.squeeze().detach().cpu().numpy()))

                    # --- (optional) debug saving ---
                    out_p_np = np.transpose(out_p.squeeze(0).detach().cpu().numpy(), (1, 2, 0))
                    out_np = (out_p_np > 0.5)

                    pbar.set_postfix(dict(
                        label_cls=set(list(label_np.flatten().astype(int))),
                        pred_cls=set(list(out_np.flatten().astype(int)))
                    ))

                    if save_dir is not None:
                        slice_name = slice_path[0].split("/")[-1]
                        save_cond_path   = os.path.join(save_dir, f"{slice_name.split('.')[0]}-input.{slice_name.split('.')[-1]}")
                        save_gt_path     = os.path.join(save_dir, f"{slice_name.split('.')[0]}-gt.{slice_name.split('.')[-1]}")
                        save_pred_path   = os.path.join(save_dir, f"{slice_name.split('.')[0]}-pred.{slice_name.split('.')[-1]}")
                        save_logits_path = os.path.join(save_dir, f"{slice_name.split('.')[0]}-logits.{slice_name.split('.')[-1]}")
                        save_all_path    = os.path.join(save_dir, f"all-{slice_name.split('.')[0]}.{slice_name.split('.')[-1]}")

                        save_pred   = (out_np * 255).astype(np.uint8)
                        save_logits = (out_p_np * 255).astype(np.uint8)
                        save_gt     = (label_np * 255).astype(np.uint8)
                        save_cond   = ((image_np + 1) / 2 * 255).astype(np.uint8)

                        save_pred   = overlay_images(save_cond, colorize_multichannel_segment(save_pred))
                        save_logits = overlay_images(save_cond, colorize_multichannel_segment(save_logits))
                        save_gt     = overlay_images(save_cond, colorize_multichannel_segment(save_gt))
                        save_all    = np.concatenate((save_cond, save_gt, save_pred, save_logits), axis=1)

                        Image.fromarray(save_cond).save(save_cond_path)
                        Image.fromarray(save_gt).save(save_gt_path)
                        Image.fromarray(save_pred).save(save_pred_path)
                        Image.fromarray(save_logits).save(save_logits_path)
                        Image.fromarray(save_all).save(save_all_path)

                    # log for latent/image grids occasionally
                    if pbar.total >= 8 and (pbar.n % max(1, (pbar.total // 8)) == 0):
                        encoder_posterior = self.encode_first_stage(
                            torch.from_numpy(label_np).unsqueeze(0).permute((0, 3, 1, 2)).half().cuda()
                        )
                        label_latent = self.get_first_stage_encoding(encoder_posterior).detach()
                        label_latent_list.append(label_latent)

                        samples_latent_list.extend(samples_pred[0:])
                        label_image_list.append(torch.from_numpy(colorize(label_np, num_classes=self.num_classes)).unsqueeze(0).permute(0, 3, 1, 2))
                        samples_image_list.append(torch.from_numpy(colorize(out_np,   num_classes=self.num_classes)).unsqueeze(0).permute(0, 3, 1, 2))
                        samples_logits_list.append(torch.from_numpy(out_p_np * 255).unsqueeze(0).permute(0, 3, 1, 2))
                        samples_cond_list.append(torch.from_numpy((image_np + 1) / 2 * 255).unsqueeze(0).permute(0, 3, 1, 2))

                    # MONAI metrics (dice/iou/tiou)
                    self.val_dice(out, label)
                    self.val_iou(out, label)
                    self.val_tiou(out, label)
                ...
                try:
                    seg_label_pair = [
                        self.prepare_latent_to_log(
                            torch.cat((torch.cat(samples_latent_list, dim=0), torch.cat(label_latent_list, dim=0)), dim=1).float()
                        ),
                        (torch.cat((
                            torch.cat(samples_logits_list, dim=3),
                            torch.cat(samples_image_list, dim=3),
                ...
                # ---- Dice/IoU/tIoU prints ----
                val_dice_means = self.val_dice.aggregate("none")
                class_dices = []
                for idx in range(0, self.num_classes):
                    dv = val_dice_means[:, idx].nanmean().item()
                    class_dices.append(dv)
                    print(f"\033[31m[Mean Dice][cls {idx}]: {dv}\033[0m")
                mean_dice = sum(class_dices) / len(class_dices)
                print(f"\033[31m[Mean Dice]: {mean_dice}\033[0m")

                # HS+IS mean (assuming cls1 & cls2 are stroke classes)
                if self.num_classes >= 3:
                    mean_dice_hsis = float(np.nanmean([class_dices[1], class_dices[2]]))
                    print(f"\033[31m[Mean Dice (HS+IS)][{self.__class__.__name__.lower()}]: {mean_dice_hsis}\033[0m")

                val_iou_means = self.val_iou.aggregate("none")
                class_ious = []
                for idx in range(0, self.num_classes):
                    iv = val_iou_means[:, idx].nanmean().item()
                    class_ious.append(iv)
                    print(f"\033[31m[Mean  IoU][cls {idx}]: {iv}\033[0m")
                mean_iou = sum(class_ious) / len(class_ious)
                print(f"\033[31m[Mean IoU]: {mean_iou}\033[0m")
                if self.num_classes >= 3:
                    mean_iou_hsis = float(np.nanmean([class_ious[1], class_ious[2]]))
                    print(f"\033[31m[Mean  IoU (HS+IS)][{self.__class__.__name__.lower()}]: {mean_iou_hsis}\033[0m")

                val_tiou_means = self.val_tiou.aggregate("none")
                class_tious = []
                for idx in range(0, self.num_classes):
                    tv = val_tiou_means[:, idx].nanmean().item()
                    class_tious.append(tv)
                    print(f"\033[31m[Mean  tIoU][cls {idx}]: {tv}\033[0m")
                mean_tiou = sum(class_tious) / len(class_tious)
                print(f"\033[31m[Mean tIoU]: {mean_tiou}\033[0m")
                if self.num_classes >= 3:
                    mean_tiou_hsis = float(np.nanmean([class_tious[1], class_tious[2]]))
                    print(f"\033[31m[Mean  tIoU (HS+IS)][{self.__class__.__name__.lower()}]: {mean_tiou_hsis}\033[0m")

                # ---- Per-class Sens/Spec/HD95 prints (ignore_empty) ----
                for cls in (1, 2):
                    ms = float(np.nanmean(sens_list_by_cls[cls])) if len(sens_list_by_cls[cls]) else float("nan")
                    mp = float(np.nanmean(spec_list_by_cls[cls])) if len(spec_list_by_cls[cls]) else float("nan")
                    mh = float(np.nanmean(hd95_list_by_cls[cls])) if len(hd95_list_by_cls[cls]) else float("nan")
                    print(f"\033[31m[Mean Sensitivity][cls {cls}]: {ms}\033[0m")
                    print(f"\033[31m[Mean Specificity][cls {cls}]: {mp}\033[0m")
                    print(f"\033[31m[Mean HD95][cls {cls}]: {mh}\033[0m")

                ms_u = float(np.nanmean(sens_list_fg)) if len(sens_list_fg) else float("nan")
                mp_u = float(np.nanmean(spec_list_fg)) if len(spec_list_fg) else float("nan")
                mh_u = float(np.nanmean(hd95_list_fg)) if len(hd95_list_fg) else float("nan")
                print(f"\033[31m[Mean Sensitivity (cls1+cls2)]: {ms_u}\033[0m")
                print(f"\033[31m[Mean Specificity (cls1+cls2)]: {mp_u}\033[0m")
                print(f"\033[31m[Mean HD95 (cls1+cls2)]: {mh_u}\033[0m")

                return class_dices, class_ious, class_tious, ms_u, mp_u, mh_u, seg_label_pair
        ...
        # ---- run direct ----
        ema_dice, ema_iou, ema_tiou, ema_sens_u, ema_spec_u, ema_hd95_u, seg_label_pair = get_dice(
            data, used_sampler="direct", save_dir=save_dir, ddim_steps=ddim_steps
        )

        multi_dice, multi_iou, multi_tiou = np.array(ema_dice), np.array(ema_iou), np.array(ema_tiou)

        metrics_dict.update({"val_avg_dice/direct_ema": float(np.nanmean(multi_dice))})
        metrics_dict.update({"val_avg_iou/direct_ema":  float(np.nanmean(multi_iou))})
        metrics_dict.update({"val_avg_tiou/direct_ema": float(np.nanmean(multi_tiou))})

        for cls in range(0, self.num_classes):
            metrics_dict.update({f"val_avg_dice/direct_ema_{cls}": float(multi_dice[cls])})
            metrics_dict.update({f"val_avg_iou/direct_ema_{cls}":  float(multi_iou[cls])})
            metrics_dict.update({f"val_avg_tiou/direct_ema_{cls}": float(multi_tiou[cls])})
        ...
        # union (cls1+cls2) scalars
        metrics_dict.update({"val_avg_sensitivity/direct_ema": float(ema_sens_u)})
        metrics_dict.update({"val_avg_specificity/direct_ema": float(ema_spec_u)})
        metrics_dict.update({"val_avg_hd95/direct_ema":        float(ema_hd95_u)})

        # monitor
        metrics_dict.update({"val_avg_dice": list(multi_dice)})
        metrics_dict.update({"val_avg_iou":  list(multi_iou)})
        metrics_dict.update({"val_avg_tiou": list(multi_tiou)})
        metrics_dict.update({"val_avg_sensitivity": float(ema_sens_u)})
        metrics_dict.update({"val_avg_specificity": float(ema_spec_u)})
        metrics_dict.update({"val_avg_hd95":        float(ema_hd95_u)})
