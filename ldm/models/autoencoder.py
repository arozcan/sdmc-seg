...
class AutoencoderKL(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if "dice_wob_mean" in self.monitor:
            dice_all_mean, dice_wob_mean, iou_all_mean, iou_wob_mean, log_dice_iou = self.loss_dice_iou(inputs, reconstructions, posterior, 0, self.global_step, split="train")
            self.log("dice_all_mean", dice_all_mean, prog_bar=True, logger=True, on_step=True, on_epoch=True)
            self.log("dice_wob_mean", dice_wob_mean, prog_bar=True, logger=True, on_step=True, on_epoch=True)
            self.log("iou_all_mean", iou_all_mean, prog_bar=True, logger=True, on_step=True, on_epoch=True)
            self.log("iou_wob_mean", iou_wob_mean, prog_bar=True, logger=True, on_step=True, on_epoch=True)
            self.log_dict(log_dice_iou, prog_bar=False, logger=True, on_step=True, on_epoch=False)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if "dice_wob_mean" in self.monitor:
            dice_all_mean, dice_wob_mean, iou_all_mean, iou_wob_mean, log_dice_iou = self.loss_dice_iou(inputs, reconstructions, posterior, 0,
                    self.global_step, split="train")
            self.log("val/dice_all_mean", dice_all_mean, prog_bar=True, logger=True, on_step=True, on_epoch=True)
            self.log("val/dice_wob_mean", dice_wob_mean, prog_bar=True, logger=True, on_step=True, on_epoch=True)
            self.log("val/iou_all_mean", iou_all_mean, prog_bar=True, logger=True, on_step=True, on_epoch=True)
            self.log("val/iou_wob_mean", iou_wob_mean, prog_bar=True, logger=True, on_step=True, on_epoch=True)
            self.log_dict(log_dice_iou, prog_bar=False, logger=True, on_step=True, on_epoch=False)
    ...
    def prepare_latent_to_log(latent):
        ...
        latent_grid = rearrange(latent, 'n b c h w -> b n c h w')
        latent_grid = rearrange(latent_grid, 'b n c h w -> (b n) c h w')
        return make_grid(latent_grid, nrow=latent_grid.shape[0])
    ...
    # def loss_dice_iou(self, inputs, reconstructions, posteriors, optimizer_idx,
    ...
    #     inputs_binary = apply_threshold_to_channels(inputs)
    #     reconstructions_binary = apply_threshold_to_channels(reconstructions)
    ...
    #     dice_metric_all(y_pred=reconstructions_binary, y=inputs_binary)
    #     dice_metric_wob(y_pred=reconstructions_binary, y=inputs_binary)
    #     iou_metric_all(y_pred=reconstructions_binary, y=inputs_binary)
    #     iou_metric_wob(y_pred=reconstructions_binary, y=inputs_binary)
    ...
    #     log = {"{}/dice_all_mean".format(split): dice_metric_all_mean, 
    #            "{}/dice_wob_mean".format(split): dice_metric_wob_mean,
    #            "{}/iou_all_mean".format(split): iou_metric_all_mean, 
    #            "{}/iou_wob_mean".format(split): iou_metric_wob_mean
    #                }
    #     return dice_metric_all_mean, dice_metric_wob_mean, iou_metric_all_mean, iou_metric_wob_mean, log
    

    def loss_dice_iou(self, inputs, reconstructions, posteriors, optimizer_idx,
                  global_step, split="train"):
        # Sınıf sayısını al (kanal sayısı)
        num_classes = reconstructions.shape[1]

        # Yumuşak çıktıları argmax ile sınıf etiketine çevir
        reconstructions_argmax = torch.argmax(torch.softmax(reconstructions, dim=1), dim=1)  # [B, H, W]
        inputs_argmax = torch.argmax(inputs, dim=1)  # Eğer input da one-hot ise

        # One-hot'a geri çevir
        reconstructions_binary = torch.nn.functional.one_hot(reconstructions_argmax, num_classes).permute(0, 3, 1, 2).float()
        inputs_binary = torch.nn.functional.one_hot(inputs_argmax, num_classes).permute(0, 3, 1, 2).float()
        ...
        log = {
            f"{split}/dice_all_mean": dice_metric_all_mean,
            f"{split}/dice_wob_mean": dice_metric_wob_mean,
            f"{split}/iou_all_mean": iou_metric_all_mean,
            f"{split}/iou_wob_mean": iou_metric_wob_mean
        }

        return dice_metric_all_mean, dice_metric_wob_mean, iou_metric_all_mean, iou_metric_wob_mean, log
    
    def compress_latent(self, img):
        """
        Gürültülü latent kanalını -1 ila 1 aralığına sıkıştırır ve [0,1]'e normalize eder.
        """
        img = torch.tanh(img)        # aralığı -1 ile +1'e sıkıştırır
        img = (img + 1.0) / 2.0      # [0, 1] aralığına getirir
        return img

    def robust_normalize(self, img, lower=1, upper=99):
        flat = img.flatten()
        lower_val = torch.quantile(flat, lower / 100.0)
        upper_val = torch.quantile(flat, upper / 100.0)
        img = torch.clamp(img, lower_val, upper_val)
        img = (img - lower_val) / (upper_val - lower_val + 1e-5)
        return img

    def loss_dice_iou_all(self, dataloader, max_batches=None, max_batches_image=None, outpath=None):
        self.eval()  # Evaluation moduna al
        device = next(self.parameters()).device  # Modelin bulunduğu cihaz
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(dataloader):
                ...
                inputs = self.get_input(batch, self.image_key).to(device)
                reconstructions, posterior = self(inputs)

                dice_all, dice_wob, iou_all, iou_wob, _ = self.loss_dice_iou(
                    inputs, reconstructions, posterior,
                    optimizer_idx=0, global_step=0, split="eval"
                )

                total_dice_all += dice_all
                total_dice_wob += dice_wob
                total_iou_all += iou_all
                total_iou_wob += iou_wob
                total_batches += 1

                # 🔍 Görsel loglama
                if (max_batches is None or batch_idx < max_batches) and (outpath is not None):
                    os.makedirs(outpath, exist_ok=True)
                    log = self.log_images(batch, only_inputs=False)

                    for key, img_tensor in log.items():
                        img = img_tensor[0].detach().cpu()  # İlk örneği al
                        if key == "reconstructions":
                            # Softmax → Argmax → Sınıf maskesi
                            img = torch.softmax(img.unsqueeze(0), dim=1)[0]  # [C, H, W]
                            class_map = torch.argmax(img, dim=0)  # [H, W]

                            # Renk eşleme tablosu (class 0: kırmızı, class 1: yeşil, class 2: mavi)
                            ...
                            # Sınıf haritasını RGB görüntüye dönüştür
                            img_rgb = class_colors[class_map]  # [H, W, 3]
                            img_rgb = img_rgb.permute(2, 0, 1)  # [3, H, W] → to_pil_image için

                            img = TF.to_pil_image(img_rgb)
                        elif img.ndim == 3 and img.shape[0] in [1, 3]:  # [C, H, W]
                            img = TF.to_pil_image(img.clamp(-1, 1) * 0.5 + 0.5)  # Normalize edilmişse
                        elif img.ndim == 2:  # grayscale
                            img = self.robust_normalize(img)  # veya robust_normalize(img)
                            img = TF.to_pil_image(img.unsqueeze(0))
                        else:
                            print(f"Geçersiz görüntü boyutu: {img.shape} - {key}")
                            continue
                        img.save(os.path.join(outpath, f"{key}_{batch_idx}.png"))

        if total_batches == 0:
            raise RuntimeError("Dataloader is empty.")

        return {
            "avg_dice_all": total_dice_all / total_batches,
            "avg_dice_wob": total_dice_wob / total_batches,
            "avg_iou_all": total_iou_all / total_batches,
            "avg_iou_wob": total_iou_wob / total_batches,
